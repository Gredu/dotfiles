Config {
    font = "xft:Cozette-13px",
    bgColor = "#1D1F21",
    fgColor = "#c5c8c6",
    position = Top,
    commands = [
        Run MultiCpu ["-t","cpu: <total>%","-L","30","-H","60","-h","#FFB6B0","-l","#CEFFAC","-n","#FFFFCC","-w","3"] 10,
        Run Memory ["-t","mem: <usedratio>%","-H","8192","-L","4096","-h","#FFB6B0","-l","#CEFFAC","-n","#FFFFCC","-w","3"] 10,
        Run Swap ["-t","swap: <usedratio>%","-H","1024","-L","512","-h","#FFB6B0","-l","#CEFFAC","-n","#FFFFCC","-w","3"] 10,
        Run DynNetwork ["-t","<rx> \8595 <tx> \8593","-S","True","-d","1","-H","1048576","-L","65536","-h","#FFB6B0","-l","#CEFFAC","-n","#FFFFCC","-m","9","-c"," "] 10,
        Run Date "%a %b %_d %H:%M" "date" 10,
        Run DiskU [("/", "/ <used>/<size>"), ("/home", "~ <usedp>%")]
            ["-L", "20", "-H", "50", "-m", "1", "-p", "3", "-h", "#FFB6B0", "-l", "#CEFFAC", "-n", "#FFFFCC"]
            600,
        Run UnsafeStdinReader
    ],
    sepChar = "%",
    alignSep = "}{",
    template = "%UnsafeStdinReader% }{ %multicpu%   %memory%   %swap%   %disku%   %dynnetwork%   <fc=#FFFFCC>%date%</fc>"
}
